...
export default function CharactersShowcase() {
  // Ana karakterleri örnek verilerle tanımlıyorum - sadece Eren, Mikasa ve Armin
  ...
  return (
    <section className="py-16 md:py-24 min-h-screen flex flex-col justify-center relative">
      ...
      <div className="absolute top-10 right-10 w-40 h-40 bg-gradient-to-br from-[#FF4655]/10 to-transparent rounded-full filter blur-3xl opacity-60"></div>
      <div className="absolute bottom-20 left-20 w-48 h-48 bg-gradient-to-tr from-blue-500/10 to-transparent rounded-full filter blur-3xl opacity-40"></div>
      
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 my-auto">
        <div className="mb-10 text-center relative">
          <div className="inline-block mb-3">
            <span className="text-xs font-medium text-[#FF4655] tracking-widest uppercase bg-[#FF4655]/10 px-4 py-1 rounded-full backdrop-blur-sm">Karakterler</span>
          </div>
          
          <h2 className="text-3xl md:text-4xl font-bold mb-3">
            <span className="text-[#FF4655]">Ana</span> Karakterler
            <div className="h-1 w-16 bg-[#FF4655] mx-auto mt-2 mb-3"></div>
          </h2>
          
          <p className="text-gray-300 max-w-2xl mx-auto backdrop-blur-sm bg-[#0F1923]/20 p-3 rounded-lg">
            Attack on Titan'ın hikayesini şekillendiren ve dünyayı değiştiren karakterleri keşfedin.
          </p>
        </div>
        
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 md:gap-8 mx-auto max-w-7xl">
          {mainCharacters.map((character) => (
            <div 
              key={character.id}
              className="group relative overflow-hidden cursor-pointer transition-all duration-500 h-full transform hover:-translate-y-2"
              onMouseEnter={() => setHoveredCharacter(character.id as number)}
              onMouseLeave={() => setHoveredCharacter(null)}
            >
              {/* Ana kart konteyneri - Glass morphic tasarım */}
              <div className={`relative rounded-2xl overflow-hidden bg-[#1A242D]/30 backdrop-blur-xl border border-white/10 shadow-lg group-hover:shadow-xl ${
              ...
              } transition-all duration-500 h-full hover:border-white/20`}>
                ...
                {/* Status durumu */}
                {character.status && (
                  <div className={`absolute top-3 right-3 z-20 px-3 py-1 rounded-full text-xs font-medium backdrop-blur-xl border ${
                ...
                {character.species && character.species.length > 0 && (
                  <div className="absolute top-3 left-3 z-20 px-3 py-1 rounded-full text-xs font-medium bg-[#1A242D]/50 backdrop-blur-xl text-gray-300 border border-white/10 shadow-md">
                    {character.species[0]}
                  </div>
                )}
                
                {/* Karakter bilgileri - kartın en altında, hover durumunda yukarı kayacak */}
                <div className="absolute bottom-0 left-0 right-0 p-4 z-30 transition-all duration-300 group-hover:translate-y-[-60px] bg-gradient-to-t from-[#1A242D]/80 to-transparent backdrop-blur-sm">
                ...
                <Link href={character.path} className="absolute bottom-0 left-0 right-0 p-4 z-30 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-4 group-hover:translate-y-0">
                  <button 
                    type="button"
                    className="w-full py-2 bg-gradient-to-r from-[#FF4655]/80 to-[#FF4655] text-white rounded-md text-sm font-medium flex items-center justify-center shadow-[0_4px_8px_rgba(0,0,0,0.5)] border border-[#FF4655]/30"
        ...
        <div className="mt-12 text-center">
          <Link 
            href="/characters"
            className="inline-flex items-center px-6 sm:px-8 py-3 sm:py-4 bg-[#1A242D]/30 backdrop-blur-xl text-[#FF4655] font-medium rounded-md hover:bg-[#FF4655]/10 transition-all duration-300 border border-white/10 hover:border-[#FF4655]/30 shadow-character hover:shadow-[0_10px_40px_rgba(255,70,85,0.1)]"
          >
